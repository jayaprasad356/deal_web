...
<script type="text/javascript">
	function search1() {
	    ...
	    var sea=document.getElementById("search").value;
            ...
            {
                   window.location.href = "search.php?sea="+sea;
            }
	}	
</script>
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="ifsc-add.php" class="btn btn-primary btn-sm">Add New</a>
	</div>
</section>
...
<?php if (isset($_GET['page']) && $_GET['page']!="") {
	...
	$result_count = mysqli_query($dbconn,"SELECT COUNT(*) As total_records FROM `bank_details` WHERE bank_name LIKE '%$sea%' OR bank_ifsc LIKE '%$sea%' OR bank_branch LIKE '%$sea%' OR bank_address LIKE '%$sea%' OR bank_city LIKE '%$sea%' OR bank_district LIKE '%$sea%' OR bank_state LIKE '%$sea%' OR bank_micr LIKE '%$sea%'");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = mysqli_query($dbconn,"SELECT * FROM `bank_details` WHERE bank_name LIKE '%$sea%' OR bank_ifsc LIKE '%$sea%' OR bank_branch LIKE '%$sea%' OR bank_address LIKE '%$sea%' OR bank_city LIKE '%$sea%' OR bank_district LIKE '%$sea%' OR bank_state LIKE '%$sea%' OR bank_micr LIKE '%$sea%' LIMIT $offset, $total_records_per_page");
		...
		echo "<tr>
			  ...
			  <td>".$row['bank_name']."</td>
	 		  <td>".$row['bank_ifsc']."</td>
				...
				<td>".$row['bank_micr']."</td>
				<td>".$row['bank_district']."</td>
				<td>".$row['bank_state']."</td>
				<td><a href=ifsc-edit.php?id=".$row['bank_id']."><button class='btn btn-primary btn-xs'>Edit</button></a>
				<a href='#' class='btn btn-danger btn-xs' data-href='ifsc-delete.php?id=".$row['bank_id']."' data-toggle='modal' data-target='#confirm-delete'>Delete</a></td>
		   	  </tr>";
        }
	mysqli_close($dbconn);
    ?>
