...
<section class="content-header">
	<div class="content-header-left">
		<h1>View Bank Swift BIC Details</h1>
	</div>
...
<?php if (isset($_GET['page']) && $_GET['page']!="") {
	...
	$result_count = mysqli_query($dbconn,"SELECT COUNT(*) As total_records FROM `swift_details`");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = mysqli_query($dbconn,"SELECT * FROM `swift_details` LIMIT $offset, $total_records_per_page");
		...
		echo "<tr>
			  ...
			  <td>".$row['bank']."</td>
	 		  <td>".$row['swift_code']."</td>
				...
				<td>".$row['address']."</td>
				<td>".$row['country']."</td>
				<td><a href=swift-edit.php?id=".$row['id']."><button class='btn btn-primary btn-xs'>Edit</button></a>
				<a href='#' class='btn btn-danger btn-xs' data-href='swift-delete.php?id=".$row['id']."' data-toggle='modal' data-target='#confirm-delete'>Delete</a></td>
		   	  </tr>";
        }
	mysqli_close($dbconn);
    ?>
...
?>
	...
	<a <?php if($page < $total_no_of_pages) { echo "href='?page=$next_page'"; } ?>>Next</a>
	</li>
