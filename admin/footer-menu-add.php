...
if(isset($_POST['form_page'])) {
    ...
    if( $_POST['footer_menu_parent'] == '') {
        $valid = 0;
        $error_message .= "You must have to select a parent for this footer Menu<br>";
    }

    if($valid == 1) {

        // saving into the database
        $statement = $pdo->prepare("INSERT INTO tbl_footer_menu (footer_menu_type,page_id,footer_menu_name,footer_menu_url,footer_menu_order,footer_menu_parent) VALUES (?,?,?,?,?,?)");
        $statement->execute(array('Page',$_POST['page_id'],'','',$_POST['footer_menu_order'],$_POST['footer_menu_parent']));

        $success_message = 'footer Menu is added successfully.';
    }
...
if(isset($_POST['form_other'])) {
    ...
    if(empty($_POST['footer_menu_name'])) {
        $valid = 0;
        $error_message .= "footer Menu Name can not be empty<br>";
    }
    if(empty($_POST['footer_menu_url'])) {
        $valid = 0;
        $error_message .= "footer Menu URL can not be empty<br>";
    }
    if(empty($_POST['footer_menu_order'])) {
        $valid = 0;
        $error_message .= "footer Menu Order can not be empty<br>";
    ...
    if( $_POST['footer_menu_parent'] == '') {
        $valid = 0;
        $error_message .= "You must have to select a parent for this footer_menu<br>";
    }
    ...
    if($valid == 1) {
        ...
        // saving into the database
        $statement = $pdo->prepare("INSERT INTO tbl_footer_menu (footer_menu_type,page_id,footer_menu_name,footer_menu_url,footer_menu_order,footer_menu_parent) VALUES (?,?,?,?,?,?)");
        $statement->execute(array('Other',0,$_POST['footer_menu_name'],$_POST['footer_menu_url'],$_POST['footer_menu_order'],$_POST['footer_menu_parent']));
							...
							<div class="box box-info">
								<div class="box-body">
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<select class="form-control select2" name="footer_menu_parent">
                                                ...
                                                foreach ($res as $row) {
                                                    if($row['page_id']==0) {
                                                        echo '<option value="'.$row['footer_menu_id'].'">'.$row['footer_menu_name'].'</option>';
                                                    } else {
                                                        ...
                                                        foreach ($res1 as $row1) {
                                                            echo '<option value="'.$row['footer_menu_id'].'">'.$row1['page_name'].'</option>';
                                                        }
                                                    }
                                                }
                                                ?>
									...
									<div class="form-group">
										...
										<div class="col-sm-1">
											<input type="text" class="form-control" name="footer_menu_order">
										</div>
									</div>
									<div class="form-group">
							...
							<div class="box box-info">
								<div class="box-body">
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="text" class="form-control" name="footer_menu_name">
										</div>
									</div>
									<div class="form-group">
